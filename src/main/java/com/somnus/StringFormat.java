...
public class StringFormat {
	...
	@Test
	public void test1(){
	    System.out.println(String.format("Hi, [%s]", "Somnus"));
	...
	public void test2(){
	    ...
	    System.out.println(String.format("HH:MM格式(24时制)：%tR%n", date));
	}
	/*
	3、格式化日期字符串
	b或者h: 月份简称，如
	...
	public void test4(){
	    ...
	    System.out.println(String.format("2位数字12时制的小时(前面不补0):%tl%n",date));
	    System.out.println(String.format("2位数字的分钟(不足2位前面补0):%tM%n",date));
	    System.out.println(String.format("2位数字的秒(不足2位前面补0):%tS%n",date));
	    System.out.println(String.format("3位数字的毫秒(不足3位前面补0):%tL%n",date));
	    System.out.println(String.format("9位数字的毫秒数(不足9位前面补0):%tN%n",date));
	    System.out.println(String.format(Locale.US,"小写字母的上午或下午标记(英)：%tp",date));                          // 输出字符串变量str的内容
